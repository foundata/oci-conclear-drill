...
drill_usage() {
  printf 'usage: %s --workspace <dir> [--wheel <path>] [--registry <namespace>] [--profile <name>]\n' "${0##*/}" >&2
  exit 64
}

drill_parse_arguments() {
  DRILL_WORKSPACE=""; DRILL_WHEEL=""; DRILL_REGISTRY="quay.io/conclear-drill"; DRILL_PROFILE="drill"
  while [ "$#" -gt 0 ]; do
    case "$1" in
